...
if (url.searchParams.has('socks5api') && url.searchParams.get('socks5api') !== '') {
    ...
    socks5s = socksend !== undefined ? socks5s.slice(socksstart, socksend) : socks5s.slice(socksstart);
                ...
                const links = socks5s.map(socks5带地址 => {
                    ...
                    // 从URL参数获取要删除的关键字(delt)和替换文本(addt)
                    const delt = url.searchParams.get('delt') || '';  // 默认空字符串（不删除）
                    const addt = url.searchParams.get('addt') || '';  // 默认空字符串（直接删除不替换）

                    const 落地国家 = socks5带地址.split('#').length > 1 
                        ? socks5带地址.split('#')[1].replace(new RegExp(delt, 'g'), addt)  // 动态替换
                        : host名称;
